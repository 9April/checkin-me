...
export async function executeSaveBooking(
  ...
  try {
    ...
    try {
      ...
      // Decode the client-side pixel-perfect A4 PDF stay agreement
      let pdfAttachment: any = undefined;
      const agreementPdfDataUri = formData.get('agreementPdf') as string;
      if (agreementPdfDataUri) {
        try {
          console.log('--- Processing client-side generated A4 PDF stay agreement ---');
          const base64Data = agreementPdfDataUri.split(',')[1] || agreementPdfDataUri;
          ...
          console.log('--- A4 PDF attachment successfully prepared from Client-side capture ---');
        } catch (decErr) {
          console.error('--- Non-Fatal: Failed to decode client-side generated PDF:', decErr);
        }
      }
